...
func main() {
	...
	// ─── SERVER MODE WITH LIVE HTTP INGESTION GATEWAY ────────────────────────
	if len(os.Args) == 1 {
		...
		// Real API Route for Stress Testing
		http.HandleFunc("/api/v1/execute", func(w http.ResponseWriter, r *http.Request) {
			...
			taskID := fmt.Sprintf("stress-%d", time.Now().UnixNano()%100000)
			...
			_ = qm.PushTask(ctx, task)
			w.WriteHeader(http.StatusAccepted)
			w.Write([]byte(fmt.Sprintf(`{"task_id":"%s","status":"queued"}`, taskID)))
		})

		fmt.Println("[Gateway] Ingestion API listening on http://localhost:2112/api/v1/execute")

		// Boot 3 Workers to process incoming tasks
